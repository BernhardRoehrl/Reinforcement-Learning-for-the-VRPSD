...
avg_distance = []
avg_list = []
capacity = 10  # Vehicle Capacity Parameter
Length = 5
...
def create_customer():
    ...
    for i in range(1, Length, 1):
        customer_x = Customer(random.randrange(1, 3, 1), i)
...
class Service:
    ...
    def refill(self, next_customer):
        ...
        self.position = 0
        self.capacity = capacity
        self.distance += data['distance_matrix'][self.position][next_customer]
        self.step_distance += data['distance_matrix'][self.position][next_customer]
        self.position_update(next_customer)
        self.capacity = self.capacity - customer_list[self.position].demand
        customer_list[self.position].demand = 0
        return self.position, self.capacity, self.distance, customer_list[self.position].demand, self.step_distance

    def serve(self, next_customer):
        if self.capacity < customer_list[next_customer].demand:
            """Failure 1: serve partially -> Go To depot -> Go back -> Fulfill serving"""
            """Hinfahren1 und customer partially bedienen"""
            self.distance += data['distance_matrix'][self.position][next_customer]
            self.step_distance += data['distance_matrix'][self.position][next_customer]
            self.position_update(next_customer)
            customer_list[next_customer].demand = customer_list[next_customer].demand - self.capacity
            self.capacity = 0
            ...
            self.step_distance += data['distance_matrix'][self.position][next_customer]
            self.position_update(next_customer)
            self.capacity = self.capacity - customer_list[next_customer].demand
            customer_list[next_customer].demand = 0
            return self.position, self.capacity, self.distance, customer_list[next_customer].demand, self.step_distance
        else:
            """Vehicle can serve fully. regular workflow"""
            self.capacity = self.capacity - customer_list[next_customer].demand
            customer_list[next_customer].demand = 0
            self.distance_update(next_customer)
            self.position_update(next_customer)
            return customer_list[next_customer].demand, self.capacity, self.position, self.distance, self.step_distance
...
state_position = len(customer_list)
state_capacities = capacity
q_table = np.zeros((capacity+1, Length+1, 10,  len(action_space_size)))
...
if __name__ == "__main__":
    ...
    for episode in range(num_episodes):
        ...
        state = 0
        old_capacity = 0
        while not all (customer.demand ==0 for customer in customer_list):
            ...
            if exploration_rate_threshold < exploration_rate:
                ...
                valid_customers = [customer.position for customer in customer_list if customer.demand != 0]
                next_customer = random.choice(valid_customers)
                if action == 1:
                    """Agent chooses action 1: Vehicle must serve"""
                    vehicle.serve(next_customer)
                    exploration_counter += 1
                else:
                    """Agent chooses action 2: refilling"""
                    vehicle.refill(next_customer)
                    exploration_counter += 1
            # Else doing a random choice --> exploration
            else:
                action = np.argmin(q_table[old_capacity, old_position, state, :])
                next_customer = np.argmin(q_table[old_capacity, :, state, action])
                if action == 1:
                    vehicle.serve(next_customer)
                elif action == 0:
                    vehicle.refill(next_customer)
            ...
            if state == Length + 1:
                reward = 3000
                old_value = q_table[old_capacity, old_position, state, action]
                best_expected_value = np.min(q_table[vehicle.capacity, vehicle.position, step+1, :])
                bellman_term = (reward + discount_rate * best_expected_value - old_value)
                q_table[old_capacity, old_position, action] = old_value + learning_rate * bellman_term
                break
            else:
            ...
            old_value = q_table[old_capacity, old_position, state, action]
            best_expected_value = np.min(q_table[vehicle.capacity, vehicle.position, state+1, :])
            bellman_term = (reward + discount_rate * best_expected_value - old_value)
            q_table[old_capacity, old_position,state, action] = old_value + learning_rate * bellman_term
